...
const shift = computed(() => ((Math.round(props.params.shift ?? 3) % 26) + 26) % 26)
const text = computed<string>(() => props.options.text ?? 'MEET ME AT NOON')
const enc = (s: string, k: number) => s.toUpperCase().replace(/[A-Z]/g, (c) => A[(A.indexOf(c) + k) % 26])
const cipher = computed(() => enc(text.value, shift.value))
const C = { x: 240, y: 240 }
  ...
  <div>
    <svg ref="svg" :viewBox="`0 0 ${W} ${H}`" class="block w-full touch-none select-none" @pointerdown="dragging = true; svg?.setPointerCapture($event.pointerId); pick($event)" @pointermove="dragging && pick($event)" @pointerup="dragging = false" @pointercancel="dragging = false">
      ...
      <g class="num" font-size="18">
        ...
        <text v-for="(ch, i) in text.split('')" :key="'p' + i" :x="500 + i * 19" y="180" text-anchor="middle" fill="var(--fg)">{{ ch }}</text>
        <g v-for="(ch, i) in text.split('')" :key="'a' + i"><line v-if="ch !== ' '" :x1="500 + i * 19" y1="192" :x2="500 + i * 19" y2="224" stroke="var(--line)" /></g>
        <text x="500" y="250" fill="var(--muted)" font-size="12">{{ t('cipher.cipher') }}</text>
        <text v-for="(ch, i) in cipher.split('')" :key="'c' + i" :x="500 + i * 19" y="280" text-anchor="middle" fill="var(--accent)" font-weight="600">{{ ch }}</text>
      </g>
    </svg>
