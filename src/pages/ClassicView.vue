...
// Translations of each simulation's own words, compiled from content/classic/text/<lang>/<id>.yaml.
// Only text is swapped inside the frame; the simulation's code, canvas and layout are untouched.
const texts = import.meta.glob('../generated/classic-text.*.json')
async function sendText() {
  const lang = locale.value
  const load = texts[`../generated/classic-text.${lang}.json`]
  if (lang === 'en' || !load) return
  const all = ((await load()) as { default: Record<string, Record<string, string>> }).default
  const dict = all[props.id]
  if (!dict) return
  const dir = document.documentElement.dir === 'rtl' ? 'rtl' : 'ltr'
  frame.value?.contentWindow?.postMessage({ type: 'vi-text', dict: { ...dict }, lang, dir }, location.origin)
}
function onMessage(e: MessageEvent) {
  ...
  if (e.data?.type === 'vi-ready') (ready.value = true), sendTheme(), void sendText()
}
