...
export function displayTex(token: string): string {
  const sample: Record<string, string> = { '\\dot': '\\dot{x}', '\\ddot': '\\ddot{x}', '\\hat': '\\hat{p}', '\\bar': '\\bar{x}', '\\vec': '\\vec{v}', '\\mathbf': '\\mathbf{v}', '\\mathbb': '\\mathbb{R}', '\\frac': '\\frac{a}{b}', '\\sqrt': '\\sqrt{x}', '\\binom': '\\binom{n}{k}', '\\sum': '\\sum_{i=1}^{n}', '\\int': '\\int_a^b', '\\lim': '\\lim_{x \\to a}', '\\partial': '\\partial', '\\langle': '\\langle v \\rangle', '\\text': '\\text{text}', '\\mathrm': '\\mathrm{kg}', '\\operatorname': '\\operatorname{Var}', "'": "f'", '!': 'n!', d: 'dx', '\\arg': '\\arg\\max' }
